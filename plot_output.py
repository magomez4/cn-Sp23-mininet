...
class Row():
    ...
    def __str__(self):
        return f'{str(self.interval_end):>6} sec {self.cwnd:>6} KBytes {self.bandwidth:>6} Gbits/sec'
    ...
    def _parse_cwnd(self, line):
        '''Cwnd'''
        m = re.search('([0-9\.]+)\s+?(KBytes)', line)
        self.cwnd = float(m.group(1)) if m else None
        self.cwnd_unit = m.group(2) if m else None
...
def plot_congestion(algo):
    ...
    s2 = [row.__dict__ for row in read_output_file(f'{algo}/s2.txt', offset=250)]
    df = pd.DataFrame(data=s1)
    ax = df.plot(kind='line', 
            x='interval_end', 
            y='cwnd', 
            xlabel='Time in (sec)', 
            ylabel=f'Congestion Window in ({s1[0]["cwnd_unit"]})',
            style='-')
